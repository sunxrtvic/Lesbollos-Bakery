...
//Filtramos los productos de la tabla "eco" para solo tomar los que su stock sea mayor a 0
$products = [];
$result = $mysqli->query("SELECT id, nombre, precio, stock FROM eco WHERE stock > 0 LIMIT $offset, $productos_por_pagina");
...
$total_resultados = $mysqli->query("SELECT COUNT(*) as total FROM eco WHERE stock > 0")->fetch_assoc()['total'];
                        ...
                        <?php if (!isset($_SESSION['usuario']) || $_SESSION['usuario']['tipo'] !== 'admin'): ?>
                            ...
                            <?php if (isset($_SESSION['usuario']) && $_SESSION['usuario']['tipo'] === 'admin'): ?>
                                <form method="GET" action="editar-producto.php" style="display:inline;">
                                    ...
                                    <input type="hidden" name="tabla" value="eco">
